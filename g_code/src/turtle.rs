...
impl<'input> GCodeTurtle<'input> {
    ...
    pub fn round_coordinate(&self, val: f64) -> f64 {
    ...
    fn line_to(&self, to: Point<f64>) -> Vec<Token<'input>> {
        command!(LinearInterpolation {
            X: self.round_coordinate(to.x),
            Y: self.round_coordinate(to.y),
            F: self.feedrate,
        })
        .into_token_vec()
    }
    ...
    fn circular_interpolation(&self, svg_arc: SvgArc<f64>) -> Vec<Token<'input>> {
        ...
        match (svg_arc.flags.large_arc, svg_arc.flags.sweep) {
            (false, true) => command!(CounterclockwiseCircularInterpolation {
                X: self.round_coordinate(svg_arc.to.x),
                Y: self.round_coordinate(svg_arc.to.y),
                R: self.round_coordinate(svg_arc.radii.x),
                F: self.feedrate,
            })
            .into_token_vec(),
    ...
    /// Appends only the drawing commands for a stroke.
    ///
    /// This intentionally omits rapid positioning, tool sequences, and SVG
    /// hierarchy comments so callers can wrap the trajectory in their own
    /// machine-specific process cycle.
    pub fn draw_stroke_commands(&mut self, stroke: Stroke) {
        self.append_draw_commands(stroke, false);
    }

    fn append_draw_commands(&mut self, stroke: Stroke, include_comments: bool) {
        for command in stroke.into_commands() {
            match command {
                        ...
                        FlattenWithArcs::flattened(&svg_arc, self.arc_flattening_tolerance())
                            ...
                            .for_each(|segment| match segment {
                                ...
                                ArcOrLineSegment::Line(line) => {
                                    if include_comments {
                                        // Preserve the existing generic SVG-to-G-code behavior.
                ...
                DrawCommand::Comment(comment) if include_comments => {
    ...
    fn stroke(&mut self, stroke: Stroke) {
        ...
        self.program.append(
            &mut command!(RapidPositioning {
                X: self.round_coordinate(start.x),
                Y: self.round_coordinate(start.y),
            })
            .into_token_vec(),
        );
        self.tool_on();

        self.append_draw_commands(Stroke::new(start, commands.collect()), true);
    }
