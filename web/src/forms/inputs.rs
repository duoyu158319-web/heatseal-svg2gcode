...
use crate::{
    state::{AppState, FormState, HeatSealSettings},
    ui::*,
};
...
macro_rules! heat_input {
    ($name:ident, $label:literal, $desc:literal, $field:ident, $validate:expr) => {
        paste! {
            #[function_component([<$name Input>])]
            pub fn [<$name:snake:lower _input>]() -> Html {
                let app_state = use_store_value::<AppState>();
                ...
                html! {
                    <FormGroup success={form_state.$field.is_ok()}>
                        <Input<f64, String>
                            label=$label
                            desc=$desc
                            default={app_state.heat_seal.$field}
...
heat_input!(
    DwellSeconds,
    "Heat-seal dwell time (s)",
    "Global dwell time used by every heat-seal cycle",
    dwell_seconds,
...
heat_input!(
    ...
    "Global M104 temperature used by every heat-seal cycle",
    temperature,
    HeatSealSettings::validate_temperature
);
heat_input!(
    WorkingHeight,
    "Heat-seal working height (mm)",
    "Global Z working height used by every heat-seal cycle",
    working_height,
    HeatSealSettings::validate_height
);
