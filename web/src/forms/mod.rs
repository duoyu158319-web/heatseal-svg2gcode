...
use gloo_file::{
    ...
    futures::{read_as_bytes as read_as_bytes_async, read_as_text},
};
    ...
    let save_onclick = {
        ...
        app_dispatch.reduce_mut_callback(move |app| {
            if !disabled {
                ...
                app.heat_seal = form_state.heat_seal_settings();
...
pub fn import_export_modal() -> Html {
    ...
    let import_state = use_state(|| Option::<Result<WebSettings, String>>::None);
    ...
    let export_onclick = {
        ...
        app_dispatch.reduce_mut_callback(move |app| {
            match serde_json::to_vec_pretty(&WebSettings::new(
                app.settings.clone(),
                app.heat_seal.clone(),
            )) {
        ...
        Callback::from(move |file_list: FileList| {
            ...
            import_reading_setter.clone().set(Some(read_as_bytes(
                ...
                move |res| {
                    ...
                    match res {
                        Ok(WebSettings { version, .. }) if version > WEB_SETTINGS_VERSION => {
                            import_state.set(Some(Err(format!(
                                "Your web settings use a newer wrapper version: {version}. Latest supported: {WEB_SETTINGS_VERSION}."
                            ))));
                        }
                        Ok(WebSettings { settings: Settings { version: Version::Unknown(unknown), .. }, .. }) => {
                        ...
                        Ok(mut web_settings) => {
                            let old_version = web_settings.settings.version.clone();
                            import_state.set(Some(
                                if let Err(msg) = web_settings.heat_seal.validate() {
                                    Err(format!("Invalid heat-seal settings: {msg}"))
                                } else if let Err(msg) = web_settings.settings.try_upgrade() {
                                    Err(format!(
    ...
    let import_save_onclick = {
        ...
        let close_ref = close_ref.clone();
        app_dispatch.reduce_mut_callback(move |app| {
            if let Some(Ok(web_settings)) = import_state.as_ref() {
                app.settings = web_settings.settings.clone();
                app.heat_seal = web_settings.heat_seal.clone();
                // App only hydrates the form on start now, so need to do it again here
                form_dispatch
                    .reduce_mut(|form| *form = FormState::from_app(&app.settings, &app.heat_seal));
        ...
        <Modal
            ...
            header={html!(
                <>
                    ...
                    <p>{"Exports versioned web settings including heat-seal parameters. Original svg2gcode settings JSON files can still be imported."}</p>
                </>
            )}
            body={
                html!(
...
#[function_component(MarkerReplacementForm)]
pub fn marker_replacement_form() -> Html {
    let app_dispatch = use_dispatch::<AppState>();
    let svg_upload_state = use_state(|| Option::<Result<(), String>>::None);
    let gcode_upload_state = use_state(|| Option::<Result<(), String>>::None);

    let svg_upload_onchange = {
        let svg_upload_state = svg_upload_state.clone();
        app_dispatch.future_callback_with(move |app, file_list: FileList| {
            let svg_upload_state = svg_upload_state.clone();
            Box::pin(async move {
                ...
                let filename = file.name();
                let result = read_as_text(&gloo_file::File::from(file))
                    .await
                    .map_err(|err| format!("Error reading {filename}: {err}"))
                    .and_then(|text| {
                ...
                let svg = result.as_ref().ok().cloned();
                svg_upload_state.set(Some(result.map(|_| ())));
                app.reduce_mut(|app| app.replacement_svg = svg);
            })
        })
    };

    let gcode_upload_onchange = {
        let gcode_upload_state = gcode_upload_state.clone();
        app_dispatch.future_callback_with(move |app, file_list: FileList| {
            let gcode_upload_state = gcode_upload_state.clone();
            Box::pin(async move {
                let Some(file) = file_list.item(0) else {
                ...
                let filename = file.name();
                let result = read_as_bytes_async(&gloo_file::File::from(file))
                    .await
                    .map_err(|err| format!("Error reading {filename}: {err}"))
                    .and_then(|bytes| {
                        let had_utf8_bom = bytes.starts_with(&[0xEF, 0xBB, 0xBF]);
                        let content_bytes = if had_utf8_bom { &bytes[3..] } else { &bytes };
                        String::from_utf8(content_bytes.to_vec())
                            .map_err(|err| format!("{filename} is not valid UTF-8: {err}"))
                            .map(|content| GCodeTemplate {
                                content,
                                filename,
                                had_utf8_bom,
                            })
                    });
                let template = result.as_ref().ok().cloned();
                gcode_upload_state.set(Some(result.map(|_| ())));
                app.reduce_mut(|app| app.gcode_template = template);
            })
        })
    };
    ...
    html! {
        <div class="columns">
            <div class="column col-6 col-sm-12">
                <FormGroup success={svg_upload_state.as_ref().map(Result::is_ok)}>
                    <FileUpload<(), String>
                        label="Select one SVG file for replacement"
                        desc="Converted with the same heat-seal settings and SVG pipeline"
                        accept=".svg,image/svg+xml"
                        multiple={false}
                        onchange={svg_upload_onchange}
                        parsed={(*svg_upload_state).clone()}
                    />
                </FormGroup>
            </div>
            <div class="column col-6 col-sm-12">
                <FormGroup success={gcode_upload_state.as_ref().map(Result::is_ok)}>
                    <FileUpload<(), String>
                        ...
                        parsed={(*gcode_upload_state).clone()}
                    />
                </FormGroup>
            </div>
        </div>
